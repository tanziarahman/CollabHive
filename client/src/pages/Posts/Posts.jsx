...
export default function Posts() {
  ...
  const [inviteContext, setInviteContext] = useState(null); // { project, role }
  ...
  const openInvite = async (project, role) => {
    ...
    try {
      ...
      const demoUsers = [
        { _id: "u1", fullName: "Sarah Ahmed", skills: ["React", "CSS", "Figma"] },
        { _id: "u2", fullName: "Rafiq Islam", skills: ["Node.js", "Express", "MongoDB"] },
        { _id: "u3", fullName: "Meherin Chowdhury", skills: ["React", "JavaScript"] },
        { _id: "u4", fullName: "Tanvir Hossain", skills: ["Python", "FastAPI"] },
        { _id: "u5", fullName: "Nusrat Jahan", skills: ["React", "Node.js"] },
      ];
      const filtered = demoUsers
        .map((u) => ({
          ...u,
          matchingSkills: u.skills.filter((s) => required.includes(s)),
        }))
        .filter((u) => u.matchingSkills.length > 0);
  ...
  const handleInvite = async (candidateId) => {
    ...
    try {
      ...
      // Backend endpoint not built yet — invite still marked as sent locally
    } finally {
  ...
  const anyOverlayOpen = Boolean(activeProject) || Boolean(inviteContext);
  ...
  return (
    <>
      ...
      <div className={`my-posts-page ${anyOverlayOpen ? "is-blurred" : ""}`}>
      ...
      {inviteContext && (
        <div className="applicants-overlay" onClick={closeInvite}>
          <div className="applicants-modal" onClick={(e) => e.stopPropagation()}>
            <div className="applicants-modal-header">
              <div>
                <h2>Invite for {inviteContext.role}</h2>
                <p>{inviteContext.project.title}</p>
              </div>

              <button className="close-btn" onClick={closeInvite}>
                ✕
              </button>
            </div>
            ...
            <div className="applicants-list">
              {loadingCandidates ? (
                <div className="no-applicants">Finding matching people...</div>
              ) : inviteCandidates.length > 0 ? (
                inviteCandidates.map((candidate) => (
                  <div className="applicant-row" key={candidate._id}>
                    <div className="applicant-info">
                      <span className="applicant-name">{candidate.fullName}</span>
                      <span className="applicant-role">
                        {(candidate.matchingSkills || candidate.skills || []).join(", ")}
                      </span>
                    </div>
                    ...
                    <div className="applicant-actions">
                      <Link
                        ...
                        className="view-link"
                        state={{ inviteProjectId: inviteContext.project._id }}
                      >
                        View Profile
                      </Link>

                      {invitedIds.includes(candidate._id) ? (
                        <span className="decision-badge accepted">Invited</span>
                      ) : (
                        <button
                          className="accept-btn"
                ...
                <div className="no-applicants">
                  No matching people found for this role's skills yet.
                </div>
              )}
            </div>
          </div>
